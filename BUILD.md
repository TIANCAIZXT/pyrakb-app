...
3. **下载**：点进该次运行 → 底部 **Artifacts** 区域 → 下载 `pyrakb-windows-latest`（里面是 `Mini Wiki_0.2.1_x64_en-US.msi`）。同理 `pyrakb-macos-latest` 是 `.dmg`。
...
npx tauri build
# 手动压 dmg（headless 可用）
hdiutil create -volname "Mini Wiki" -srcfolder "src-tauri/target/release/bundle/macos/Mini Wiki.app" -ov -format UDZO "src-tauri/target/release/bundle/dmg/Mini Wiki_0.2.1.dmg"
```

产物：`src-tauri/target/release/bundle/dmg/Mini Wiki_0.2.1.dmg`（本机为 `_x64`，在 Apple Silicon 上经 Rosetta 2 运行，Intel 原生运行）。
...
6. 在该目录执行：
   ...
   npm run build
   ```
7. 产物：`src-tauri\target\release\bundle\msi\Mini Wiki_0.2.1_x64_en-US.msi`
