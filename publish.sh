#!/usr/bin/env bash
# Mini Wiki 一键发布脚本
# 作用：把本地 pyrakb-app 仓库推到 GitHub 并发布 v0.2.1 Release，上传 macOS DMG，
...
TAG="v0.2.1"
TITLE="Mini Wiki v0.2.1"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$SCRIPT_DIR"

# DMG 路径：默认本地构建产物（npx tauri build + make_dmg.sh 生成）
DMG_PATH="${DMG_PATH:-$SCRIPT_DIR/src-tauri/target/release/bundle/dmg/Mini Wiki_0.2.1_x64.dmg}"
if [ ! -f "$DMG_PATH" ]; then
  echo "✗ 找不到安装包：$DMG_PATH"
  echo "  请先执行：npx tauri build && ./make_dmg.sh"
  exit 1
fi
...
- macOS: Mini Wiki_0.2.1_x64.dmg（双击安装；首次若被拦截，右键「打开」→「仍要打开」）
...
echo "✅ 完成！Release 地址："
echo "   $RELEASE_URL"
echo "   Skill 现在可从该 Release 拉取 Mini Wiki_0.2.1_x64.dmg。"
