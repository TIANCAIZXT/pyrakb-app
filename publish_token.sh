#!/usr/bin/env bash
# Mini Wiki 免 gh 一键发布脚本（用 GitHub REST API + git 内嵌令牌）
# 适用于本机装不了 gh 的情况。用法：
...
UP_HTTP=$(curl -s -o /dev/null -w "%{http_code}" -X POST \
  ...
  "$API/repos/$LOGIN/$REPO_NAME/releases/$REL_ID/assets?name=Mini Wiki_0.1.3_x64.dmg")
